...
The computed ``translation`` and ``rotation`` can then be used to recover the configuration of the template with maximal similarity to the target. This is functionally similar to the operation performed using :py:meth:`tme.density.Density.match_densities` and :py:meth:`tme.density.Density.match_structure_to_density`.
