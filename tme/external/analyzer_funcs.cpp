...
void absolute_minimum_deviation(
    ...
    py::ssize_t n = coordinates.shape(0);
    py::ssize_t k = coordinates.shape(1);
    py::ssize_t ik, jk, in, jn;
...
inline py::array_t<int, py::array::c_style> find_candidate_indices(
    ...
    for (py::ssize_t i = 1; i < n; ++i) {
        ...
        for (int candidate_index : candidate_indices) {
            jk = static_cast<py::ssize_t>(candidate_index) * k;
...
py::array_t<T, py::array::c_style> find_candidate_coordinates(
    ...
    py::ssize_t num_candidates = candidate_indices_array.shape(0);
    py::ssize_t k = coordinates.shape(1);
    auto coordinates_data = coordinates.data();
    ...
    for (py::ssize_t i = 0; i < num_candidates; ++i) {
        py::ssize_t candidate_index = static_cast<py::ssize_t>(candidate_indices_data[i]) * k;
        std::copy(
